...
class RandomExplainer:
    ...
    def explain_instance(self, instance_vector, label, classifier, num_features,
                         dataset):
        nonzero = instance_vector.nonzero()[1]
        explanation = np.random.choice(nonzero, num_features)
        return [(x, 1) for x in explanation]

    def explain(self, train_vectors, train_labels, classifier, num_features,
                dataset):
...
class GeneralizedLocalExplainer:
    ...
    def data_labels_distances_mapping(self, raw_data, classifier_fn):
        data, labels, distances, mapping = self.data_labels_distances_mapping_fn(
            raw_data, classifier_fn, self.num_samples)
        return data, labels, distances, mapping
    ...
    def explain_instance_with_data(self, data, labels, distances, label,
                                   num_features):
        ...
        alpha = 1
        if self.lambda_:
            classif = linear_model.Lasso(alpha=self.lambda_,
                                         fit_intercept=False,
                                         positive=self.positive)
        ...
        if self.verbose:
            print 'Prediction_local', debiased_model.predict(data[
                0, used_features].reshape(1, -1)) + mean, 'Right:', labels[
                    0, label]
        if self.return_mean:
            return sorted(
                zip(used_features, debiased_model.coef_),
    ...
    def explain_instance(self,
        ...
        data, labels, distances, mapping = self.data_labels_distances_mapping(
            raw_data, classifier_fn)
        if self.return_mapped:
            if self.return_mean:
                exp, mean = self.explain_instance_with_data(
                    data, labels, distances, label, num_features)
            else:
                exp = self.explain_instance_with_data(data, labels, distances,
                                                      label, num_features)
            ...
            if self.return_mean:
                ...
                return exp
        return self.explain_instance_with_data(data, labels, distances, label,
                                               num_features), mapping
