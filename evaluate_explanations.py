...
class ExplanationEvaluator:
    ...
    def init_classifiers(self, dataset):
        ...
        for classifier in self.classifier_names:
            if classifier == 'l1logreg':
                ...
                for c in try_cs:
                    self.classifiers[dataset][
                        'l1logreg'] = linear_model.LogisticRegression(
                            penalty='l1', fit_intercept=True,
                            C=c)
                    self.classifiers[dataset]['l1logreg'].fit(
                        self.train_vectors[dataset], self.train_labels[dataset])
                    lengths = [
                        len(x.nonzero()[0])
                        for x in self.classifiers[dataset][
                            'l1logreg'].transform(self.train_vectors[dataset])
                    ]
                    if np.max(lengths) <= 10:
                        ...
                        break
            if classifier == 'tree':
                self.classifiers[dataset]['tree'] = tree.DecisionTreeClassifier(
                    random_state=1)
                self.classifiers[dataset]['tree'].fit(
                    self.train_vectors[dataset], self.train_labels[dataset])
                lengths = [len(get_tree_explanation(self.classifiers[dataset][
                    'tree'], self.train_vectors[dataset][i]))
                           for i in range(self.train_vectors[dataset].shape[0])]
    ...
    def load_datasets(self, dataset_names):
        ...
        for dataset in dataset_names:
            self.train_data[dataset], self.train_labels[
                dataset], self.test_data[dataset], self.test_labels[
                    dataset], _ = LoadDataset(dataset)
    ...
    def vectorize_and_train(self):
        ...
        for d in self.train_data:
            ...
            self.train_vectors[d] = self.vectorizer[d].fit_transform(
                self.train_data[d])
            self.test_vectors[d] = self.vectorizer[d].transform(self.test_data[
                d])
        ...
        for d in self.train_data:
            ...
            for c in self.classifiers[d]:
                ...
                for i in range(len(self.test_data[d])):
                    if c == 'l1logreg':
                        true_features = set(
                            [x for x in self.test_vectors[d][i].nonzero()[1]
                             if x in c_features])
                    elif c == 'tree':
                        true_features = get_tree_explanation(
                            self.classifiers[d][c], self.test_vectors[d][i])
                    if len(true_features) == 0:
                        continue
                    to_get = budget
                    exp_features = set(map(lambda x: x[0], explain_fn(
                        self.test_vectors[d][i], self.test_labels[d][
                            i], self.classifiers[d][c], to_get, d)))
                    test_results[d][c].append(
                        float(len(true_features.intersection(exp_features))) /
                        len(true_features))
...
def main():
    ...
    if args.explainer == 'lime':
        ...
        sigmas = {'multi_polarity_electronics': {'tree': 0.5,
                                                 'l1logreg': 1},
                  'multi_polarity_kitchen': {'tree': 0.75,
                                             'l1logreg': 2.0},
                  'multi_polarity_dvd': {'tree': 8.0,
                                         'l1logreg': 1},
                  'multi_polarity_books': {'tree': 2.0,
                                           'l1logreg': 2.0}}
    ...
    out = {'train': train_results[dataset][algorithm],
